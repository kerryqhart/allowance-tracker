...
fn generator_path_coverage() {
    ...
    // realistic case count. `duplicate_dropped_is_reachable_and_logged`
    // below reaches it by construction instead, and this test still reports
    // its organic count (expected: 0) for visibility.
}

// ===========================================================================
// DuplicateDropped: only reachable by construction, so construct it.
//
// This decision fires only when a re-keyed row's hash-derived id happens to
// already be taken by another surviving row -- a 64-bit hash collision.
// Reaching that by pure random generation is infeasible in any realistic
// budget (merge::tests plants it manually for exactly this reason). This
// test plants the same kind of collision, but randomises every value that
// isn't load-bearing for producing it, so it stays a real generative test
// rather than a single fixed example.
// ===========================================================================

/// FNV-1a exactly as `merge::content_suffix` (private to that module, so
/// duplicated here). If the real algorithm ever changes, this copy drifts
/// and the collision below stops firing -- that surfaces as a hard failure
/// in `duplicate_dropped_is_reachable_and_logged`, not a silent hole.
fn predicted_content_suffix(row: &TxRow) -> String {
...
/// `wins()` is private to `merge`; duplicated here (a five-line total
/// function, low drift risk) purely to predict, at construction time, which
/// side's add/add row will be the one re-keyed.
fn predicted_wins(ours: &Provenance, theirs: &Provenance) -> bool {
...
proptest! {
    ...
    fn duplicate_dropped_is_reachable_and_logged(
        ...
        ours_oid in OURS_OID_DOMAIN,
        theirs_oid in THEIRS_OID_DOMAIN,
    ) {
        let ours_row = make_row("p".to_string(), 0, ours_cents, "slime".to_string());
        let theirs_row = make_row("p".to_string(), 0, theirs_cents, "book".to_string());
        ...
        // Whichever side LOSES the add/add tiebreak is the one re-keyed to
        // "p-<hash of the losing row>". Plant an orphan at exactly that id
        // so the post-loop dedupe must collide the two and log the drop.
        let ours_wins = predicted_wins(&ours_prov, &theirs_prov);
        let loser_row = if ours_wins { &theirs_row } else { &ours_row };
        let collision_id = format!("p-{}", predicted_content_suffix(loser_row));
        let orphan = make_row(collision_id.clone(), 1, orphan_cents, "should not vanish".to_string());
        ...
        let ours = Sided { rows: ours_rows, provenance: ours_prov };
        let theirs = Sided { rows: theirs_rows, provenance: theirs_prov };
