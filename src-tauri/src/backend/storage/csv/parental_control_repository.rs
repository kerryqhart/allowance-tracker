...
impl crate::backend::storage::ParentalControlStorage for ParentalControlRepository {
    async fn record_parental_control_attempt(&self, child_id: &str, attempted_value: &str, success: bool) -> Result<i64> {
        ...
        let next_id = self.get_next_id(&child_directory).await?;
        ...
        self.append_parental_control_attempt(&child_directory, &record).await?;
        
        info!("Recorded parental control attempt for child ID '{}': {} (success: {})", 
              child_id, attempted_value, success);
        Ok(next_id)
    }
    ...
    async fn get_all_parental_control_attempts(&self, limit: Option<u32>) -> Result<Vec<DomainParentalControlAttempt>> {
        ...
        for directory in directories {
            let attempts = self.load_parental_control_attempts_from_directory(&directory, None).await?;
        ...
        info!("Listed {} total parental control attempts across all children", all_attempts.len());
        Ok(all_attempts)
    }
}
